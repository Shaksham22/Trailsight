...
Trailsight V2 keeps the proven V1 technical foundation—Python, DuckDB, FastAPI, React/TypeScript/Vite, one stdio MCP server, OpenAI Agents SDK, deterministic evidence, structured AI output, JSONL telemetry, pytest, and one Docker runtime—but replaces the V1 transaction-case domain with an account/network-native AML investigation domain.
...
flowchart TB
    ...
    API["FastAPI runtime\nREST, AI orchestration, evidence validation"]
...
8. **Keep runtime local and small operationally.** One Docker application container, one mounted analytical DuckDB, one tiny writable state path, one trace path, one external model API.
...
Owns paginated resources, review-status mutation, subject-detail endpoints, AI orchestration endpoint registration, static frontend serving, and safe error mapping.
...
Owns prompt/model configuration, tool-use orchestration, structured findings, evidence validation, bounded follow-up, evals, and telemetry.
...
| Docker one-unit pattern | REUSE | Still simplest runtime |
...
Dead V1 domain code is removed only after V2 packages are integrated and regression-tested.
...
## FINAL IMPLEMENTATION-ORIENTED AUTHORITY

The frozen runtime architecture above is unchanged. Implementation sequencing/ownership is now governed by `09_IMPLEMENTATION_ROADMAP.md` and the revised `implementation/` package set.

The inspected repository still contains active V1 packages and no production `src/trailsight_v2/` namespace. Therefore V2 production work must be created under `src/trailsight_v2/` and V1 runtime code remains in place until final Codex integration proves V2 imports/tests and removes proven-dead V1 paths.

The final package strategy intentionally supports separate repository lineages:

- backend/data/detector/domain/API/AI evolve through the R0→R5 lineage defined in `09_IMPLEMENTATION_ROADMAP.md`;
- approved frontend visual foundation can evolve independently from R0 using typed V2 mocks;
- frontend is merged with the real backend only after the V2 API/AI contracts exist;
- final repository-wide integration and V1 dead-code cleanup are Codex-owned.

The existing V1 optional-AI-router pattern and thin typed frontend client/fixture pattern may be reused as infrastructure patterns only; no V1 Case, TransXion, Person/Merchant, Synthetic Region, sender-only, V1 MCP, or V1 Evidence contract survives into active V2 behavior.



## Final implementation-oriented execution authority

The runtime architecture above is unchanged. For implementation execution, architecture/interface contracts live only in their owning `docs/v2/00`–`08` design documents; `09_IMPLEMENTATION_ROADMAP.md` owns sequencing, implementer allocation, merge lineage and universal delivery policy; and `docs/v2/implementation/` contains only runnable `WP*.md` job tickets. There is no additional shared implementation-contract file that a worker must attach or execute. A worker receives the required repository ZIP, the complete final system-design ZIP, and exactly one runnable WP file (plus approved UX/UI handoff/mockups for WP05A when needed).
