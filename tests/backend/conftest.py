...
def selected_transaction(**overrides: Any) -> dict[str, Any]:
    values: dict[str, Any] = transaction(
        10_000,
...
def build_runtime_database(
    ...
    *,
    ...
    display_name: str = "Demo 01",
    entity_types: dict[tuple[str, str], str] | None = None,
) -> Path:
    selected_row = selected_transaction() if selected is None else selected
    history_rows = list(history)
    other_rows = list(other_transactions)
    all_rows = [selected_row, *history_rows, *other_rows]
    ...
    connection.execute(
        ...
        [case_ref, display_name, selected_row["transaction_ref"]],
    )
    for row in all_rows:
        connection.execute(
            """
    ...
    configured_types = entity_types or {}
    identities = {
    ...
    for bank, account in sorted(identities):
        default_type = (
            "Person"
            if (bank, account)
            == (selected_row["from_bank"], selected_row["from_account"])
            else "Merchant"
        )
        connection.execute(
            "INSERT INTO entities VALUES (?, ?, ?)",
            [bank, account, configured_types.get((bank, account), default_type)],
        )
    connection.close()
    return path
...
def service_builder(
    ...
    def build(**kwargs: Any) -> tuple[InvestigationService, RuntimeRepository, Path]:
        nonlocal counter
        counter += 1
        path = tmp_path / f"runtime-{counter}.duckdb"
        build_runtime_database(path, **kwargs)
